...
def main():
    ...
    date_folders = list_only_folders(os.path.join('/hpc/mydata/marcus.forst/data', participant))
    # date is the folder with only numbers in the title
    date = find_earliest_date_dir(date_folders)
    
    locations = os.listdir(os.path.join('/hpc/mydata/marcus.forst/data', participant, date))
    for location in locations:
        ticks = time.time()
        video_folders = os.listdir(os.path.join('/hpc/mydata/marcus.forst/data', 
                                               participant, date, location, "vids"))
        for video in video_folders:
            path =  os.path.join('/hpc/mydata/marcus.forst/data', participant, date, location, "vids", video)
