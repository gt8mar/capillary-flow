...
def main():
    if platform.system() == 'Windows':
        seg_path = 'C:\\Users\\gt8mar\\capillary-flow\\results\\segmented\\hasty\\240410'
    else:
        seg_path = '/hpc/projects/capillary-flow/results/segmented/hasty/240410'

    # get all files in the directory
    folders = os.listdir(seg_path)
    ...
    for folder in folders:
        files = os.listdir(os.path.join(seg_path, folder))
        files = [file.replace('_contrast', '').replace('_background', '').replace('set01_','').replace('set_01_', '') for file in files]
        for file in files:
            ...
            shutil.copy(os.path.join(seg_path, folder, file), os.path.join(new_path, filename))
    # for file in files:
    #     participant, date, location, video, file_prefix = parse_filename(file)
    #     filename = file_prefix + f'_{video}_seg.png'
    #     # create the new path
