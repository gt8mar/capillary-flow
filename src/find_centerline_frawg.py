...
if platform.system() != 'Windows':
    ...
    from tools.enumerate_capillaries import enumerate_capillaries
    from tools.enumerate_capillaries2 import find_connected_components
import warnings
import pandas as pd
...
def main(path ='E:\\frawg', # 'C:\\Users\\gt8mar\\capillary-flow\\tests\\part09\\230414\\loc02', #
        verbose = True, write = True, plot=False, hasty = True):
    ...
    for file in os.listdir(segmented_folder):
        if file.endswith(".png"):
            ...
            video = file.split(' ')[1].split('.')[0]

            # Define the varying parts of the filename
            segmented_filename = file
            skeleton_filename = date + f"_{video}_skeletons.png"
            cap_map_filename = date + f"_{video}_cap_map.png"
                    ...
                    if skeleton_nums.shape[1] <= MIN_CAP_LEN:
                        ...
                        if write:
                            ...
                            for i in range(len(skeleton_coords)):
                                np.savetxt(os.path.join(output_folder, "coords", date + f'_{video}_centerline_coords_{str(i).zfill(2)}.csv'), 
                                        skeleton_data[i], delimiter=',', fmt = "%s")
                                if platform.system() == 'Windows':
                                    pass
                                else:
                                    os.makedirs('/hpc/projects/capillary-flow/results/centerlines', exist_ok=True)
                                    np.savetxt(os.path.join('/hpc/projects/capillary-flow/results/centerlines', date + f'_{video}_centerline_coords_{str(i).zfill(2)}.csv'), 
                                                skeleton_data[i], delimiter=',', fmt = "%s")
...
if __name__ == "__main__":
    ...
    main(path='E:\\part01\\230928\\locfrog', verbose = True, write = True, plot=True)
    print("--------------------")
