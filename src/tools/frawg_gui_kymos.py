...
class KymographClassifier:
    ...
    def load_image(self):
        ...
        if self.metadata_dir == 'frog':
            fps = self.get_fps(self.df.loc[self.index, 'Frog'], self.df.loc[self.index, 'Date'], self.df.loc[self.index, 'Condition'], self.df.loc[self.index, 'Capillary'])
            um_slope = self.df.loc[self.index, 'Velocity (um/s)']
        else:
        ...
        if self.current_velocity_index == 0:
            ...
            average_slope = (velocities[self.current_velocity_index] * 0.8) / fps
    ...
    def update_output_csv(self):
        ...
        output_df.loc[self.index] = [
            ...
            current_row['Capillary'],
            current_row['Velocity (um/s)'],
