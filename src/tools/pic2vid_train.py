...
def pic2vid(images, SET = 'set_01', sample = 'sample_000', color = False, compress = True):
    ...
    file_list = os.listdir(os.path.join('D:\\Marcus\\train', str(SET), str(sample)))
    if 'metadata.txt' in file_list:
        metadata_path = os.path.join('D:\\Marcus\\train', str(SET), str(sample), 'metadata.txt')
        output_path = os.path.join('D:\\Marcus\\train', str(SET), str(sample))
