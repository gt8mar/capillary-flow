...
def main(path ='F:\\Marcus\\data\\hasty_seg\\230626\\part10\\230425\\loc01', # 'C:\\Users\\gt8mar\\capillary-flow\\tests\\part09\\230414\\loc02', #
    ...
    os.makedirs(os.path.join(path, 'centerlines', 'tests', 'images'), exist_ok=True)
    os.makedirs(os.path.join(path, 'centerlines', 'tests', 'coords'), exist_ok=True)
    output_folder = os.path.join(path, 'centerlines', 'tests')
    individual_caps_folder = os.path.join(path, "segmented", "individual_caps_original")
    ...
    for file in os.listdir(segmented_folder):
        if file.endswith(".png"):
            ...
            # Define the varying parts of the filename
            # segmented_filename = file.replace('set_01', 'set01').replace('.png', '')
            segmented_filename = file_prefix + f'_{video}'
            # Read in the mask
            ...
            segmented_old = load_image_with_prefix(segmented_folder, file)

            # Check to see if any element of the array is not equal:
            if (segmented != segmented_old).any():
                print('The images are not the same')
                # Print which pixels are different
                print(np.where(segmented != segmented_old))
            ...
            segmented_old[segmented_old != 0] = 1
            ...
            for cap in os.listdir(individual_caps_folder):
                if cap.endswith(".png") and segmented_filename in cap:
                    contours[cap] = cv2.imread(os.path.join(individual_caps_folder, cap), cv2.IMREAD_GRAYSCALE)
            contours_old = find_connected_components(segmented_old)    # , verbose=False, write=write, write_path = os.path.join(output_folder, 'images', cap_map_filename)
            
            print(f'The length of contours is {len(contours.keys())} whereas the length of contours_old is {len(contours_old)}')

            # # # Plot the contours
            # # for i in range(contours_old.shape[0]):
            # #     fig, axes = plt.subplots(1,2, figsize=(10, 8), sharex=True, sharey=True)
            # #     ax = axes.ravel()
            # #     ax[0].imshow(contours[i])
            # #     ax[1].imshow(contours_old[i])
            # #     plt.savefig(os.path.join(output_folder, 'images', f'{file_prefix}_{video}_capillary_{str(i).zfill(2)}.png'))
            # #     plt.imshow(contours_old[i])
            # #     # write to testing folder
...
if __name__ == "__main__":
    ticks = time.time()
    if platform.system() == 'Windows':
        # test_path = 'C:\\Users\\gt8mar\\capillary-flow\\tests\\part09\\230414\\loc02'
        test_path = 'F:\\Marcus\\data\\part12\\230428\\loc02'
    else:
        test_path = '/hpc/projects/capillary-flow/data/part09/230414/loc01'
