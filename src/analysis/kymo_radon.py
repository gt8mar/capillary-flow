...
def find_slopes(image, method = 'ridge', verbose = False, write = False, plot_title = "Kymograph", filename = "kymograph_1.png", remove_outliers = False):
    ...
    return slopes
def find_slopes_radon(image, verbose = False):
    slopes = []
    for n in range(WINDOWS):
        window_start = n * image.shape[1]//WINDOWS
        window_end = (n+1)* image.shape[1]//WINDOWS
        window = image[:, window_start:window_end]
        kymo_raw_copy = np.copy(image)
        theta = np.linspace(0., 180., max(window.shape), endpoint=False)
        sinogram = radon(window, theta=theta, circle=False)
        peaks = peak_local_max(sinogram, min_distance=10)
        if verbose:
            plt.imshow(sinogram)
            plt.plot(peaks[:, 1], peaks[:, 0], 'r.')
            plt.show()
        slope = peaks[0][1] * (180/max(window.shape)) # unit conversion to angle in degrees
        slopes.append(slope)
        # kymo_raw_copy[:,window_start:window_end] = 255
    return slopes
...
def main(SET='set_01', sample = 'sample_000', verbose = False, write = False):
    ...
    for image in images: 
        ...
        slopes = find_slopes_radon(kymo_raw, verbose=False)  
        print(slopes)          
        # plt.plot(slopes)
        # plt.show() 
