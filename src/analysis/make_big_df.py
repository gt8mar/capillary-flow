...
def main(verbose = False):
    if platform.system() == 'Windows':
        ...
        classified_kymos_path = '/hpc/projects/capillary-flow/results/classified_kymos_real.csv'

    summary_df = pd.read_csv(path)
    # Drop SYS_BP and DIA_BP from summary_df
    summary_df = summary_df.drop(columns=['SYS_BP'])
    ...
    # Print columns shared between summary_df and metadata_df
    shared_columns = set(summary_df.columns) & set(metadata_df.columns)
    print("\nShared columns between summary_df and metadata_df:")
    print(shared_columns)

    # take 'Sex' from the combined metadata and add it to summary_df for each participant
    summary_df = pd.merge(summary_df, metadata_df, on=['Participant', 'Date', 'Location', 'Video'], how='outer')

    # set pressure to pressure_y
    summary_df['Pressure'] = summary_df['Pressure_y']
    # drop the 'Pressure_x' and 'Pressure_y' columns
    summary_df = summary_df.drop(columns=['Pressure_x', 'Pressure_y'])
    
    # Save a diagnostic CSV to check merge results
    summary_df.to_csv(os.path.join(cap_flow_path, 'merge_diagnostic.csv'), index=False)
    ...
    # create SYS_BP column and DIA_BP column from BP column
    summary_df[['SYS_BP', 'DIA_BP']] = summary_df['BP'].str.split('/', expand=True).astype(int)
