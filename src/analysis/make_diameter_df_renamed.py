...
def capillary_area_summary(directory):
    ...
    segmented_files = [f for f in os.listdir(directory) if f.endswith('.png')]
    total_files = len(segmented_files)
    
    print(f"Found {total_files} area files to process")
    
    # Process each file with progress tracking
    for i, filename in enumerate(segmented_files):
        # Display progress
        ...
        if image is None:
            print(f"\nError: Failed to load image file: {file_path}")
            continue
...
def centerline_length_radius_summary(directory):
    ...
    # Get list of CSV files and total count
    csv_files = [f for f in os.listdir(directory) if f.endswith('.csv')]
    total_files = len(csv_files)
    print(f"Found {total_files} centerline files to process")

    # Iterate over each file in the directory
    for i, filename in enumerate(csv_files):
        # Display progress
        progress = (i + 1) / total_files * 100
        print(f"\rProcessing centerline files: {i+1}/{total_files} [{progress:.1f}%]", end="")
        ...
        if filename.endswith('.csv'):
            ...
            try:
                ...
                for i in range(len(df)-1):
                    # Column 0 is x/row, Column 1 is y/column
                    distance = np.sqrt(
                        (df.iloc[i, 0] - df.iloc[i+1, 0])**2 + 
                        (df.iloc[i, 1] - df.iloc[i+1, 1])**2)
    ...
    print("\nCenterline processing complete!")
    # Convert the list to a DataFrame
