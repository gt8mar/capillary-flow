...
def main(path, verbose = True, write = True, plot=False):
    ...
    for file in os.listdir(segmented_folder):
        if file.endswith(".png"):
            ...
            # Define the varying parts of the filename
            skeleton_filename = filename + "_skeletons.png"
            cap_map_filename = filename + "_cap_map.png"
                    ...
                    if skeleton_nums.shape[1] <= MIN_CAP_LEN:
                        ...
                        if write:
                            ...
                            for i in range(len(skeleton_coords)):
                                np.savetxt(os.path.join(output_folder, "coords", filename + '_centerline_coords_{str(i).zfill(2)}.csv'), 
                                        skeleton_data[i], delimiter=',', fmt = "%s")
                                if platform.system() == 'Windows':
                                    pass
                                else:
                                    os.makedirs('/hpc/projects/capillary-flow/results/centerlines', exist_ok=True)
                                    np.savetxt(os.path.join('/hpc/projects/capillary-flow/results/centerlines', filename + '_centerline_coords_{str(i).zfill(2)}.csv'), 
                                                skeleton_data[i], delimiter=',', fmt = "%s")
...
if __name__ == "__main__":
    ticks = time.time()
    main(path='E:\\frawg\\Wake Sleep Pairs\\gabby_analysis', verbose = True, write = True, plot=True)
    print("--------------------")
